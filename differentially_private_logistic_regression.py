...
class LogisticRegression:
    def __init__(
        ...
        self.initial_theta = np.random.randn(self.n_train + 1, 1) * 0.01
        self.train_history = []
        self.cv_history = []
        self.theta_history = []
    ...
    def cost_function(self, theta, X, y):
        ...
        if self.differentially_private:  # Q.4 if Enabled
            ...
            z = np.array([np.random.laplace(0, b) for i in range(X.shape[0])]).reshape(
                -1, 1
            )  # Noise
    ...
    def gradient_descent(self, verbose):
        ...
        for i in tqdm.tqdm(range(self.n_iteration)):
            cost, grad = self.cost_function(theta, self.x_train, self.y_train)
            theta = theta - (self.alpha * grad)

            cv_cost, _ = self.cost_function(theta, self.x_val, self.y_val)
            if verbose:
                print(
                    "Iteration: {}, Train-Loss:  {}, CV-Loss:  {}".format(
                        i + 1, cost, cv_cost
                    )
                )
        ...
        print(
            "Stats: Iterations {}, Train-Loss:  {}, CV-Loss:  {}".format(
                i + 1, cost, cv_cost
            )
        )
    ...
    def alpha_vs_iteration(self, loss_threshold=0.5):
        theta = self.initial_theta
        n_iterations = 0
        ...
        while True:
            ...
            if self.verbose:
                print(
                    "Iteration: {}, Train-Loss:  {}, CV-Loss:  {}".format(
                        n_iterations + 1, cost, cv_cost
                    )
                )

    def fit(self, verbose=False):
        theta, train_history, cv_history, theta_history = self.gradient_descent(verbose)
        self.accumulate_artifacts(theta, train_history, cv_history, theta_history)

        return theta, train_history, cv_history, theta_history

    def fit_sample(self):
        theta, train_history, cv_history, theta_history = self.gradient_descent()
